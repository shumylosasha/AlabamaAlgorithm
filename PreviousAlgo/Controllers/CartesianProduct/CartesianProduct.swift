...
//  EbglishPlanet
//
//  Created by Serhiy Vysotskiy on 6/11/18.
//  Copyright © 2018 Serhiy Vysotskiy. All rights reserved.
//

import Foundation

public struct CartesianProduct<T, C: Collection>: IteratorProtocol, Sequence
    where C.Iterator.Element == T
{
    ...
    public mutating func next() -> [T]? {
        ...
        let element = self.pools.enumerated().map { index, element in
            element[self.indices[index] ]
        }
    ...
    /**
     ...
     :returns:      A cartesian product generator
     */
    public func generate() -> CartesianProduct {
        return self
    }
}
